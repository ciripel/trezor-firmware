...
with open("src/start-session.json") as f:
...
# for item in MEMORY.values():
#     if item.name() == "trezor.ui" and item.type == "dict":
#         UI_ITEM = item
#     if item.type == "module" and item.name() == "__main__":
#         MAIN_ITEM = item
...
#         continue
#     visited.add(item)
#     for ptr in item.find_pointers():
#         if ptr not in MEMORY:
#             continue
#         to_visit.append((ancestors + (item,), MEMORY[ptr]))

# # %%

# for a in ancestors:
#     print(a.name(), a.type, a.ptr)
...
min_ptr = min(item.ptrval() for item in allobjs if item.ptr != "(nil)" and not item.ptr.startswith("0x5"))
